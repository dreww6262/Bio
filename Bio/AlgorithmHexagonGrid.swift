...
class AlgorithmHexagonGrid: UIViewController, UIScrollViewDelegate, UIGestureRecognizerDelegate  {
    ...
    func resizeScrollView(numPosts: Int) {
        ...
        let centeredRec = CGRect(x: newFrame.width/2 - self.view.frame.width/2, y: newFrame.height/2 - self.view.frame.height/2, width: self.view.frame.width, height: self.view.frame.height)
        self.scrollView.scrollRectToVisible(centeredRec, animated: false)
        self.scrollView.contentOffset = centeredRec.origin
    ...
    func createImageViews() {
        ...
        var avaImageView: UIImageView = UIImageView()
        if (imageViewArray.count>0) {
            avaImageView = imageViewArray[0]
        }
        ...
        resizeScrollView(numPosts: numPosts)
        while (i < numPosts + 1) {
            ...
            let locationOfHexInOuterRing = i - 1 - 6*(ring - 1)
            let fullRing = ring * 6
            let degreeOffset: CGFloat = CGFloat(90 + ((360 / fullRing) * locationOfHexInOuterRing))
            var distance: CGFloat = (hexaDiameter * CGFloat(ring))
            ...
            if (i == 0) {
                xCoordinate = self.contentView.frame.width/2 - hexaDiameter/2
                yCoordinate = self.contentView.frame.height/2 - hexaDiameter/2
                initialX = xCoordinate
                initialY = yCoordinate
                print("ycoordinate index 0 \(initialY - 75)")
            }
            else if degreeOffset <= 180 {
                print("180: This was hit at index \(i)")
                ...
                print("270: This was hit at index \(i)")
                ...
                print("360: This was hit at index \(i)")
                ...
                print("450: This was hit at index \(i)")
            ...
            let image = UIImageView(frame: CGRect(x: xCoordinate,
                                                  y: yCoordinate,
                                                  width: hexaDiameter,
                                                  height: hexaDiameter))
