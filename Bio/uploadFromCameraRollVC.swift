            ...
            uploadTask.observe(.success) { snapshot -> Void in
                ...
                userQuery.addSnapshotListener({objects,error -> Void in
                    ...
                    if error == nil && self.firstTime == true {
                        ...
                        let hexData = HexagonStructData(resource: resourceString, type: "Photo", location: userData!.numPosts + 1, thumbResource: resourceString, createdAt: NSDate.now.description, postingUserID: usernameString, text: self.titleTxt.text, views: 0, isArchived: false, docID: hexDoc.documentID)
                        userData?.numPosts += 1
